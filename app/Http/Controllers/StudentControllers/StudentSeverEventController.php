...
class StudentSeverEventController extends \App\Http\Controllers\Controller
{
    ...
    public function homeevent($id){
        ...
        if(count($todaySubjectAttendance)>0){
            foreach($todaySubjectAttendance as $item){
                $datastring.= '<tr>
                    ...
                    elseif($item->subjectattendance=='late'){
                        $datastring.='<td class="text-center" width=10%>
                            <span class="badge bg-warning p-2 d-block">Late</span>
                        </td>';
                    }
            ...
            $datastring.= '<tr><td>You dont have Schedules for this day.<td><tr>';
        }
    ...
    public static function subjectattendanceevent($studentId,$sectionId,$blockId){

        $todaySubjectAttendance =  AttendanceReport::todaySubjectAttendanceSHS($studentId,$sectionId,$blockId);
        ...
        if(count($todaySubjectAttendance)>0){
            foreach($todaySubjectAttendance as $item){
                $datastring.= '<tr>
                    ...
                    if($item->subjectattendance=='ABSENT' && $item->classstatus=="1" || ($item->subjectattendance=='ABSENT' && $item->classstatus=="0")){
                        $datastring.='<td class="text-center" width=10%>
                            <span class="badge bg-danger p-2">Absent</span>
                        </td>';
                    }
                    elseif($item->subjectattendance=='PRESENT' && $item->classstatus=="1" || ($item->subjectattendance=='PRESENT' && $item->classstatus=="0")){
                        $datastring.='<td class="text-center" width=10%>
                            <span class="badge bg-success p-2">Present</span>
                        </td>';
                    }
                    elseif($item->subjectattendance=='LATE' && $item->classstatus=="1" ){
                        $datastring.='<td class="text-center" width=10%>
    ...
    public function tapstate($studentInfo){
        ...
        $todaySchoolAttendance = AttendanceReport::todaySchoolAttendance($studentInfo);

        $datastring = '';

        $datastring .=' <li class="list-group-item">
                            <b>Date</b> <a class="float-right  mb-0 h5 text-primary">'.\Carbon\Carbon::now()->isoFormat('MMM DD, YYYY').'</a>
                        </li>';
        ...
        if(count($todaySchoolAttendance)==0){
            ...
            if($todaySchoolAttendance[0]->intimepm != NULL && $todaySchoolAttendance[0]->intimepm != '00:00:00' && $todaySchoolAttendance[0]->intimeam == '00:00:00' ){
                $datastring .='<a class="float-right text-success h5 mb-0">'.\Carbon\Carbon::create($todaySchoolAttendance[0]->intimepm)->isoFormat('hh : mm a').'</a>';
            }
            elseif($todaySchoolAttendance[0]->intimeam != NULL && $todaySchoolAttendance[0]->intimeam != '00:00:00' ){
                $datastring .='<a class="float-right text-success h5 mb-0">'.\Carbon\Carbon::create($todaySchoolAttendance[0]->intimeam)->isoFormat('hh : mm a').'</a> ';
            }
            else{
                $datastring .='<a class="float-right text-danger h5 mb-0">00 : 00</a> ';
            }
            $datastring .='</li>
                <li class="list-group-item">
                <b>Time Out</b>';
                ...
                if($todaySchoolAttendance[0]->outtimepm != NULL && $todaySchoolAttendance[0]->outtimepm != '00:00:00'){
                    $datastring .='<a class="float-right text-success h5 mb-0">'.\Carbon\Carbon::create($todaySchoolAttendance[0]->outtimepm)->isoFormat('hh : mm a').'</a>';
                }
                elseif($todaySchoolAttendance[0]->outtimeam != NULL && $todaySchoolAttendance[0]->outtimeam != '00:00:00' ){
                    $datastring .='<a class="float-right text-success h5 mb-0">'.\Carbon\Carbon::create($todaySchoolAttendance[0]->outtimeam)->isoFormat('hh : mm a').'</a>';
                }
                else{
                    $datastring .='<a class="float-right text-danger h5 mb-0">00 : 00</a>';
                }
    ...
    //                         ->where('recieverid', $studendinfo->userid)
    //                         ->orderBy('status')
    //                         ->orderBy('created_at')
    //                         ->take(3)
    //                         ->get();
    ...
    //                 $dataString .= '<a href="/viewAnnouncement/'.$notification->id.'" class="dropdown-item">
    //                                     <div class="media">
    ...
    //                                 ->select('quarter','subjects.subjdesc')->first();
                                    
    //             if(isset($gradesDetail)){
    //                 $dataString .= '<a href="/gradeannouncement/'.$notification->id.'" class="dropdown-item">
    //                                     <div class="media">
    //                                     <div class="media-body">
    //                                     <h3 class="dropdown-item-title">';
    ...
    //                 $dataString.='</h3>'.$gradesDetail->subjdesc.' Quarter '.$gradesDetail->quarter
    ...
    //     $dataString.=' <div class="dropdown-divider"></div><a href="/viewAllAnnouncement" class="dropdown-item dropdown-footer">See All Messages</a>';
    //     $data = $dataString;
    ...
    public function loadAllNotification(){
        ...
        foreach($notifications as $key=>$notification){
            ...
            if($notification->type == '1'){
                ...
                array_push($postnotification,(object) array(
                    ...
                    LoadData::loadGrades()
                            ->where('grades.id',$notification->headerid)
                            ->select('quarter','subjects.subjdesc')->get()[0]));
            }
        }
        ...
        foreach($postnotification as $item){
            ...
            if($item->notificationInfo->type=='1'){
                ...
                $dataString.='  <a href="/viewAnnouncement/'.$item->notificationInfo->id.'">'.$item->notificationContent->name.' posted an announcement</a>
                ...
                $dataString.='  <a href="/grades">'.$item->notificationContent->subjdesc.' Quarter '.$item->notificationContent->quarter.' Grades was posted</a>
                <span class="float-right">'. Carbon::parse($item->notificationInfo->created_at)->diffForHumans(DB::select('select current_timestamp')[0]->current_timestamp,\Carbon\CarbonInterface::DIFF_RELATIVE_TO_NOW).'</span>';
            }
