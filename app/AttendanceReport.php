...
class AttendanceReport extends Model
{
    ...
    public static function schoolYearBasedAttendanceReport($studentinfo){
        // return $studentinfo->syid;
        // if($studentinfo->acadprogcode=='SHS'){
        //     $enrolledstud = DB::table('sh_enrolledstud')->where('studid',$studentinfo->id)->where('levelid',$studentinfo->enlevelid)->first();
        // }

        // else{

        //     $enrolledstud = DB::table('enrolledstud')->where('studid',$studentinfo->id)->where('levelid',$studentinfo->enlevelid)->get();
        // }
        // return $enrolledstud;

            $schoolyear =  DB::table('sy')->where('id',$studentinfo->syid)->first();
    ...
    public static function todaySubjectAttendanceSHS($studentid,$sectionid,$blockid){
        ...
        //     $attendanceArray[0]->subjcode = $blockSched->subjcode;
            
        //     $subjectattendance =  DB::table('studentsubjectattendance')
        //                         ->where('studentsubjectattendance.section_id',$sectionid)
        //                         ->where('studentsubjectattendance.subject_id',$blockSched->subjid)
        //                         ->where('student_id',$studentid)
        //                         ->where('date', $nowDate)
        //                         ->get();
        ...
        //         $attendanceArray[0]->subjectattendance = strtoupper($subjectattendance[0]->status);

        //     }
        //     else{

        //         $attendanceArray[0]->subjectattendance = "NOT CHECKED";
        //     }

        //     $now = Carbon::now('Asia/Manila')->toArray();
        //     $newSchedFormatstart =  Carbon::create( $nowDate.$blockSched->stime, 'Asia/Manila');
        //     $newSchedFormatend =  Carbon::create( $nowDate.$blockSched->etime, 'Asia/Manila');
        //     $currentTimte = Carbon::create($now['year'], $now['month'], $now['day'],$now['hour'],$now['minute'],$now['second']);
    
        //     if($currentTimte>=$newSchedFormatstart && $currentTimte<=$newSchedFormatend){
        //         $attendanceArray[0]->classstatus ="current class";
        //     }
        //     else if($currentTimte>$newSchedFormatstart){
        //         $attendanceArray[0]->classstatus ="class over";
        //     }
        //     else{
        //         $attendanceArray[0]->classstatus = $currentTimte->diff($newSchedFormatstart)->format('%Hh %Im');
        //     }

        //     $classScheds->push($attendanceArray[0]);
    ...
    public static function todaySchoolAttendance($studentid){
        ...
        return DB::table('studattendance')
                    ->where('tdate',$nowDate)
