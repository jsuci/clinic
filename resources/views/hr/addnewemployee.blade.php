...
<div class="row mb-3">
    <div class="col-md-12">
        <div class="card">
            <div class="card-body">
                <form action="/addnewemployee/{{Crypt::encrypt('addemployee')}}" method="get" class="p-2">
                    <div class="row mb-2">
                        <div class="col-md-3">
      ...
      <div class="modal-dialog modal-dialog-centered modal-sm" role="document">
          <div class="modal-content">
              ...
              <div class="modal-body">
                    ...
                    <div class="submit-section">
                        <a href="/employeeslist/dashboard" class="btn btn-secondary submit-btn float-left text-white">No</a>
                        <a href="/addnewemployee/{{Crypt::encrypt('dashboard')}}" class="btn btn-primary submit-btn float-right text-white">Yes</a>
                    </div>
              </div>
          </div>
      </div>
...
<script>
    ...
    $(document).on('change','select[name="departmentid"]', function(){
        $.ajax({
            url: '/addnewemployee/{{Crypt::encrypt('getdesignations')}}',
            type:"GET",
            dataType:"json",
            data:{
    ...
    $(document).on('change','select[name="designationid"]', function(){
        ...
        if($(this)[0].selectedOptions[0].text == 'TEACHER' || $(this)[0].selectedOptions[0].text == 'PRINCIPAL'){
            ...
            $.ajax({
                url: '/addnewemployee/{{Crypt::encrypt('getacademicprogram')}}',
                type:"GET",
                dataType:"json",
                success:function(data) {
                    $.each(data, function(key, value){
