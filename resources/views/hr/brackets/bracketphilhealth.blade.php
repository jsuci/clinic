                            ...
                            <tr>
                                <td>
                                    <div class="row">
                                        ...
                                        <div class="col-5">
                                            <input type="number" name="rangefrom" step="any" class="form-control form-control-sm" value="{{$bracket->rangefrom}}" placeholder="0.00" disabled/>
                                        </div>
                                        <div class="col-2 text-center">-</div>
                                        <div class="col-5">
                                            <input type="number" name="rangeto" step="any" class="form-control form-control-sm" value="{{$bracket->rangeto}}" placeholder="0.00" disabled/>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="row">
                                        <input type="number" name="premiumrate" step="any" class="form-control form-control-sm" value="{{$bracket->premiumrate}}" placeholder="0.00" disabled/>
                                    </div>
                                </td>
                                <td>
                                    <div class="row">
                                        <input type="number" name="fixedamount" step="any" class="form-control form-control-sm" value="{{$bracket->fixedamount}}" placeholder="0.00" disabled/>
                                    </div>
                                </td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-warning editrowfields">
...
<script>
    ...
    $(document).ready(function(){
        $('#btn-add-bracket').on('click', function(){
            var htmlview = '<tr>'+
                '<td>'+
                    ...
                    '</div>'+
                '</td>'+
                '<td><input type="number" class="form-control form-control-sm new-bracket-rate" placeholder="0.00"/></td>'+
                '<td><input type="number" class="form-control form-control-sm new-bracket-fixedamount" placeholder="0.00"/></td>'+
        ...
        $(document).on('click', '.btn-submit-new', function(){
            ...
            var bracketrate       = $(this).closest('tr').find('.new-bracket-rate').val();
            var bracketrate       = $(this).closest('tr').find('.new-bracket-rate').val();
            var bracketfixedamount = $(this).closest('tr').find('.new-bracket-fixedamount').val();
            ...
            { 
                validation=1;
                $(this).closest('tr').find('.new-bracket-rate').css('border','2px solid red')
            }else{
                $(this).closest('tr').find('.new-bracket-rate').removeAttr('style');
            }
            if(bracketfixedamount.replace(/^\s+|\s+$/g, "").length == 0)
            { 
                validation=1;
                $(this).closest('tr').find('.new-bracket-fixedamount').css('border','2px solid red')
            }else{
                $(this).closest('tr').find('.new-bracket-fixedamount').removeAttr('style');
            }
            if(validation == 0)
            {
