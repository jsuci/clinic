...
<div style="width: 100%">
    <table style="width: 100%; font-size: 12px !important;" border="1">
        <tr>
            <th rowspan="2" style="width: 2.8%;">NO</th>
            <th rowspan="2" style="width: 15%;">Name</th>
            <th rowspan="2" style="width: 4%;">Daily Rate</th>
            <th rowspan="2" style="width: 3%;"># days present</th>
            <th rowspan="2" style="width: 5%;">Basic Salary</th>
            <th rowspan="2" style="width: 6%;">Absences/<br/>Tardiness</th>
            <th rowspan="2" style="width: 6%;">Birthday/<br/>Holiday</th>
            <th rowspan="2" style="width: 6%;">Gross<br/>Salary<br/>Pay</th>
            <th rowspan="2" style="width: 6%;">Total Allowances</th>
            @if(count($standarddeductions)>0)
            <th colspan="{{count($standarddeductions)}}" style="width: 25%;">STANDARD DEDUCTIONS</th>
            @endif
            <th rowspan="2" style="width: 6%;">Total Deductions</th>
        ...
        @if(count($employees)>0)
            @foreach($employees as $key => $employee)
                <tr>
                    ...
                    <td style="text-align: right;">{{$employee->header->dailyrate}}</td>
                    <td style="text-align: center;">{{$employee->header->presentdays}}</td>
            ...
            <tr style="font-weight: bold;">
                ...
                <td style="text-align: right;">{{number_format(collect($headers)->sum('lateamount')+collect($headers)->sum('daysabsentamount'),2,'.',',')}}</td>
                <td style="text-align: center;"></td>
                <td style="text-align: right;">{{number_format($overallgrosspay,2,'.',',')}}</td>
                <td style="text-align: right;">
