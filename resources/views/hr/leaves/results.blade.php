...
<tbody>
    @foreach(collect($filedleaves)->where('display','1')->values() as $leavekey => $filedleave)
        ...
        <tr>
            ...
            <td class="pl-2">
                @if(count($filedleave->approvals)>0)
                    <div class="row">
                        <div class="col-md-12">
                            @foreach($filedleave->approvals as $eachapproval)
                                <span class="right badge @if($eachapproval->appstatus == 0) badge-warning @elseif($eachapproval->appstatus == 1) badge-success @elseif($eachapproval->appstatus == 2) badge-danger @endif border" style="font-size: 11px !important;">{{ucwords(strtolower($eachapproval->lastname))}}, {{ucwords(strtolower($eachapproval->firstname))}}</span>
                            @endforeach
                        </div>
                    </div>
                @endif
