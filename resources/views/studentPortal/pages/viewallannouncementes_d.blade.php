...
{{-- @section('pagespecificscripts')
    <script>
        let loadAllNotification = new EventSource("/loadAllNotification", {withCredentials: true});
                            ...
                            <tbody id="notificationholder">
                                @foreach($data[0]->data as $item)
                                    <tr>
                                        ...
                                        @elseif($item->type=='1')
                                        <td><a href="viewAnnouncement/{{$item->headerid}}">{{$item->announcementfirstname}}, {{$item->announcementlastname}}</a> posted an announcement "{{$item->title}}"</td>
                                        @endif
