    ...
    <tbody>
        @if(count($records[0]->tracks) > 0)
            @foreach($records[0]->tracks as $trackkey => $track)
                ...
                <tr>
                    <td style="" width="5%">{{$trackkey+1}}</td>                    
                    <td style="" width="40%" >{{$track->trackname}}</th>
                    <td style="text-align: center;">{{$track->countmale}}</td>
                    <td style="text-align: center;">{{$track->countfemale}}</th>
                    <td style="text-align: center;">{{$track->total}}</th>
                </tr>
            @endforeach
            <tr>
                <td style="" width="5%"></td>                    
                <td style="" width="40%" >TOTAL</th>
                <td style="text-align: center;">{{collect($records[0]->tracks)->sum('countmale')}}</td>
                <td style="text-align: center;">{{collect($records[0]->tracks)->sum('countfemale')}}</th>
                <td style="text-align: center;">{{collect($records[0]->tracks)->sum('total')}}</th>
            </tr>
        @endif
...
@else
    ...
    @foreach($records[0]->tracks as $track)
    @php
    $students = collect($records[0]->students)->where('trackid', $track->id)->values();
...
<table border="1" style="width: 100%; border-collapse: collapse; font-size: 11px; margin-bottom: 10px; page-break-inside: auto;">
    <thead>
        <tr>
            <th colspan="4" style="text-align:left; font-size: 15px;">{{$track->trackname}}</th>
        </tr>
    </thead>
    @if(count($students) == 0)
    <tr>
