            ...
            <tbody>
                @if(count($records[0]->strands) > 0)
                    @foreach($records[0]->strands as $strandkey => $strand)
                        ...
                        <tr>
                            <td style="text-align: center" width="5%">{{$strandkey+1}}</td>                    
                            <td style="" width="40%" >{{$strand->strandname}}</th>
                            <td style="text-align: center;">{{$strand->countmale}}</td>
                            <td style="text-align: center;">{{$strand->countfemale}}</th>
                            <td style="text-align: center;">{{$strand->total}}</th>
                        </tr>
                    @endforeach
                    <tr>
                        <td style="" width="5%"></td>                    
                        <td style="" width="40%" >TOTAL</th>
                        <td style="text-align: center;">{{collect($records[0]->strands)->sum('countmale')}}</td>
                        <td style="text-align: center;">{{collect($records[0]->strands)->sum('countfemale')}}</th>
                        <td style="text-align: center;">{{collect($records[0]->strands)->sum('total')}}</th>
                    </tr>
                @endif
...
@else
@foreach($records[0]->strands as $strand)
@php
$students = collect($records[0]->students)->where('strandid', $strand->id)->values();
...
<thead>
    <tr>
        <th colspan="4" style="text-align:left; font-size: 15px;">{{$strand->strandname}}</th>
    </tr>
</thead>
@if(count($students) == 0)
<tr>
