        ...
        <table style="width: 100%; margin-top: 10px;" border="1">
            ...
            @foreach($months as $key=>$month)
                <tr>
                    ...
                    <td style="text-align: right; padding-right: 10px;">{{$month->totalreceivables > 0 ? number_format($month->totalreceivables,2,'.',',') : '0.00'}}</td>
                </tr>
            @endforeach
            <tr>
                <th colspan="3" style="text-align: right; padding-right: 10px;">TOTAL</th>
                <th style="text-align: right; padding-right: 10px;">{{collect($months)->where('totalreceivables','<','0')->count() > 0 ? number_format(collect($months)->where('totalreceivables','>','0')->sum('totalreceivables'),2,'.',',') : number_format(collect($months)->sum('totalreceivables'),2,'.',',')}}</th>
            </tr>
        </table>   
    </body>
</html>