      ...
      <tbody>
            ...
            @foreach ($students as $item)
                  ...
                  <tr data-gs="{{$item->nogs}}"  data-id="{{$item->id}}" class="studtr studtrgrading">
                        <th class="pl-4">{{$item->lastname.', '.$item->firstname}}</th>
...
<table class="table" id="grades">
  <thead>
            <tr>
                  ...
                  <td colspan="8" style="font-size:10px; padding:4px">
                  ...
                  <td colspan="8" style="font-size:10px; padding:4px">
                        SUBJECT: {{$subjectStatus[0]->subjtitle}}
                  </td>
            ...
            <tr>
                  ...
                  <th style="text-align:left !important;padding-left: 3px !important;width:200px">Highest Possible Score</th>
                  @php
                        $withData = false;
                  @endphp
                  @if(count($gsheader->gsdget) > 0)
                          ...
                          @foreach ($grading_system_detail as $gsditem)
                                ...
                                @foreach (collect($item->gsdget)->where('gsdid',$gsditem->id) as $studgsditem)
                                      @for ($x = 1; $x <= $gsditem->items; $x++)
                                            @php
                                                  $field = 'g'.$x.'q'.$quarter;
                                                  $header = collect($item->gsdget)->where('gsdid',$gsditem->id)->first();
                                            @endphp
                                                @if($studgsditem->$field != 0)
                                                      @php
                                                            $withData = true;
...
@if(count($signatory) > 0)
      @php
            $width = 100 / ( count($signatory) * 2 );
      @endphp
      <table class="table mt-4">
            <tr>
                  @foreach($signatory as $item)
                        <td width="{{$width}}" class="text-right">{{$item->description}}</td>
                        <td width="{{$width}}" class="text-center"><u>{{$item->name}}</u></td>
                  @endforeach
            </tr>
            <tr>
                  @foreach($signatory as $item)
