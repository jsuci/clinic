...
<table style="width: 100%; font-size: 12px;">
    <tr>
        ...
        <td colspan="2" style="border: 1px solid black; text-align: center;">{{$schoolinfo->district ?? DB::table('schoolinfo')->first()->districttext ?? null }}</td>
        <td colspan="2" style="text-align: right;">Division &nbsp;</td>
        <td colspan="2" style="border: 1px solid black; text-align: center;">@if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'ndm')NORTH COTABATO @else{{$schoolinfo->division ?? DB::table('schoolinfo')->first()->divisiontext ?? null }}@endif</td>
        <td style="text-align: right;">Region &nbsp;</td>
        <td style="border: 1px solid black; text-align: center;">{{$schoolinfo->region ?? DB::table('schoolinfo')->first()->regiontext ?? null }}</td>
    </tr>
    <tr>
...
<table class="contentTable">
    <thead>
        <tr>
            ...
            <th colspan="{{$col}}" style="width:40%">(1st row for date)</th>
        ...
        <td style="width:35%;padding-left:10px;" valign="top">
                    ...
                    <td id="ada_male">
                        @php
                            if(count($currentdays) == 0)
                            {
                                ...
                                {
                                    $avedailyatt_male = number_format((collect($currentdays)->sum('presentmale')/count($currentdays)),2);
                                }else{
                    ...
                    <td id="ada_female">
                        ...
                        if(count($currentdays) == 0)
                        {
                            $avedailyatt_female = 0;
                                ...
                                if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'spct')
                                {
                                    $avedailyatt_female = number_format((collect($currentdays)->sum('presentfemale')/count($currentdays)),2);
                                }else{
            ...
            <div style="font-size: 11px; text-align:center;padding-top: 2px;">
                <center>
                    ...
                    @if(strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'sma' || strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'ndm' || strtolower(DB::table('schoolinfo')->first()->abbreviation) == 'lhs')
                    {{$teachername->firstname}} {{$teachername->middlename[0]}}. {{$teachername->lastname}} {{$teachername->suffix}}
                        @else
                        {{$teachername->firstname}} @if($teachername->middlename != null){{$teachername->middlename[0]}}.@endif {{$teachername->lastname}}@if($teachername->suffix != null) {{$teachername->suffix}}@endif @if($teachername->title != null), {{$teachername->title}}@endif
                        @endif
                    </span>
